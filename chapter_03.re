...
泉「でしょ？@<list>{name_and_age_1}はここだけ見てもわかりづらいけど、@<ttb>{他の場所で改めて変数を参照するときに何を指していたのかわからなくなる}ことのほうが大きな問題かな。しかも、@<code>{"大石泉"}はまだ見れば名前だってわかるけど、数字の@<code>{15}なんていくらでも解釈のしようがあるし、他の数と混ざったらわけがわからなくなっちゃうよね？」

颯「確かに！言われてみれば納得かもっ！」

泉「中にはごく短い名前でも問題ないようなシチュエーションもあるけど、基本的にはわかりやすい名前が推奨されるかな。とはいえ、@<chap>{chapter_03}では、とりわけ意味を持たない値については簡潔な名前で表すことにするね。ただ、こういう背景があることは忘れずに！」
...
泉「あとは画面上の『ランタイム』から『セッションを再起動する』をクリックしてもいいけど、どちらで対応するかはお好みで。ただ、セッションを再起動すると他の変数も全部消えちゃうからそこは注意だね」

颯「ねえ、これって対策する方法あるの？予約語じゃなければ普通に代入できちゃうの、怖くない？」
...
泉「こればかりはしょうがない。とはいえ、特に被りやすい名前はある程度決まっているから、今後そういったキーワードが出てきたときには紹介するね」
