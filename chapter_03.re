...
泉「◯章で簡単なコードの例として見せた、このコードは覚えてるかな？」

//list[1_add_1][1 + 1を計算するコード]{
    1 + 1
//}
...
颯「泉ちゃん、質問！『@<code>{+}』の前後にあるスペース、これって必要？@<code>{1 + 1}じゃなくて@<code>{1+1}って書いてもいいの？」

泉「どっちでも大丈夫だよ。Pythonでは、行頭でない場所に書かれたスペースは基本的に見栄えを整えるためのもので、書いても書かなくても同じように処理されるんだ@<fn>{head_space}@<fn>{space_in_str}。」

//footnote[head_space][Pythonではインデントが意味を持つため、行頭は決まった数だけスペースを書く必要があります(◯ページ参照)。]
//footnote[space_in_str][文字列の中はただの文字なので、スペースは書いた数だけスペースとして扱われます。また、本書では触れませんが、正規表現という特殊な表現方法で記述された文字列でもスペースが意味を持つことがあります。]

泉「入力できたら▶を押すかCtrl + Enterで実行してみよう。何が表示されたかな？」
...
泉「お次は引き算。……と言ってもさっきとやることは変わらない。今度はこのコードを入力してみよう。マイナスの記号『@<code>{-}』は、Shiftキーを押しながら『ほ』って書かれてあるキーを押すと入力できるよ」

//list[5_sub_2][5 - 2を計算するコード]{
    5 - 2
//}
...
泉「半角記号には限りがあるからね。このあと見ていく割り算も何となく察しがついてると思うけど、その前に掛け算を試してみよう」

//list[3_mul_7][3 * 7を計算するコード]{
    3 * 7
//}
...
泉「足し算、引き算、掛け算と来たら最後はもちろん割り算だよね。割り算の記号『@<code>{÷}』も『@<code>{×}』同様全角文字だから、代わりにスラッシュ『@<code>{/}』で割り算を行うよ。Shiftキーを押しながら『め』を押すと入力できる記号だね」
...
泉「じゃあ、Pythonでも動きを確かめてみよう。以下のコードをセルに入力して、実行してみるとどうなるかな？」

//list[10_div_2][10 / 2を計算するコード]{
    10 / 2
//}
...
泉「りあむさん、まだ早いよ。割り算は他の四則演算と違う点があるよね。次に、この数式を考えてみよう」

//list[10_div_4][10 / 4を計算するコード]{
    10 / 4
//}
...
泉「じゃあ、この数式だとどうかな？実行する前に一旦、手を止めて、どうなるか考えてみて」

//list[10_div_3][10 / 3を計算するコード]{
    10 / 3
//}

颯「10割る3……10割る3でしょ……これ、もしかして割り切れない？」

泉「そう。@<code>{10 / 3}は3.333……とどこまでも続く数で、このような小数を特に循環小数と呼ぶんだ。Colabで実行すると何が表示されたかな？」

りあむ「3.333……5！？なんじゃこりゃ！？ずっと続いてるわけじゃないし、5ってどっから出てきたんだよ！！」

泉「さらにもう一つ、3.333……のような循環小数が現れるわけでもないのに、予想に反した結果が得られる有名な例も紹介するね」

//list[0.1_add_0.2][0.1 + 0.2を計算するコード]{
    0.1 + 0.2
//}
...
//footnote[demimal_fraction][Pythonをはじめ、多くのプログラミング言語では@<code>{Decimal}という十進数(ざっくりいうと0から9までの十種類の数字で表された、我々が日常生活で扱っている数)を扱うことのできるデータがサポートされています。これを用いるとある程度は上記のような問題を回避することができますが、完璧ではありません(例えば@<code>{Decimal}で@<code>{1 / 3 * 3}を計算しても@<code>{1}には戻りません)。また、@<code>{Fraction}という分数を扱うデータを利用する方法もありますが、分数で表すことのできない値(無理数)は扱えないので注意が必要です。いずれにせよ、小数を含む計算を行いたいときは、そのときの目的に応じて適切な処理を行う必要があるのです。]
...
泉「じゃあ、実際に実行して確かめてみよう」

//list[10_div_0][10 / 0を計算するコード]{
    10 / 0
//}
...
泉「大正解！再利用したい値に何かしらの名前をつけてあげることで、以降その名前を使って繰り返し利用できるようになるんだ。このように、値に名前をつけたものを@<kw>{変数, variable}といい、変数に名前をつけることを@<kw>{代入, assignment}というんだ。変数への値の代入は、次のように記述するよ。ここで、@<code>{=}は@<kw>{代入演算子, assignment operator}と呼ばれる演算子の一種で、Shiftキーと一緒に『ほ』と書かれたキーを押すと入力できるよ」

//list[define_assignment][変数への値の代入]{
    変数の名前 = 値
//}

泉「試しに、@<code>{1 + 1}に@<code>{a}、@<code>{5 - 2}に@<code>{b}という名前をつけてみよう」

//list[assignment_sample][代入の例@<fn>{1_assign_a}]{
    a = 1 + 1
    b = 5 - 2
//}
...
泉「あってるよ。このように変数への代入を行っただけだと、セルの下には何も表示されないんだ。じゃあ今度は、実際に変数の中を覗いてみて@<fn>{variable_contents}、変数に正しく値が代入されたか確かめてみよう。まずはさっきと@<b>{同じセルに続ける形で}@<code>{print(a)}と入力して、セルを実行してみてもらえるかな？」

//footnote[variable_contents][詳しくはこのあと説明しますが、『変数の中』という表現は厳密にはやや正確性に欠けます。ただ、ほとんどのケースにおいてこのような解釈で問題ありません。]

//list[reference_sample_1][代入した値を呼び出す]{
    a = 1 + 1
    b = 5 - 2
    print(a)
//}
...
泉「じゃあ今度は、@<b>{別のセルを用意して}、そこに@<code>{print(b)}と書いて実行してみよう」

//list[reference_sample_2][代入した値を呼び出す]{
    print(b)
//}
...
//quote{
【注意】この項では発展的な内容を扱います。この後につながる重要な話なので、できれば知っておいていただきたい内容ですが、難しい場合は一旦読み飛ばしていただいて構いません。
//}
...
泉「@<kw>{オブジェクト, object}っていうのはPythonで操作するデータやプログラムの総称で、これまで出てきた@<code>{print}関数や@<code>{"Hello, world!"}という文字列、@<code>{1}や@<code>{0.2}のような数もみんなオブジェクトの一種だよ」
...
泉「じゃあ、次のプログラムを考えてみよう。まだ実行はしなくて大丈夫だよ」

//list[copy_integer][yにxを代入する]{
    x = 1
	y = x
//}
...
颯「@<code>{x}は@<code>{1}で、@<code>{y}に@<code>{x}を代入してるんだから、@<code>{y}も@<code>{1}だよね」

泉「正解！では、このコードをセルにまとめて入力して、実行してみよう」

//list[check_id_x_and_y][@<code>{x}と@<code>{y}の値と@<code>{id}を表示するコード]{
    x = 1
    y = x
    print(x)
    print(y)
...
//image[variable_02][@<code>{x}と@<code>{y}の値と@<code>{id}を表示した結果の例@<fn>{id_is_different}]{
//}

//footnote[id_is_different][後述しますが、@<code>{id(x)}や@<code>{id(y)}は必ずしもこの値になるとは限りません。ここでは、両者が同じ値である(3行目と4行目に表示された値が同じ)ことを確認してください。]

颯「@<code>{1}と@<code>{10750856}って数字が2つずつ出てきたけど……なにこれ？そもそも、@<code>{id}って何？」
...
泉「@<code>{=}(イコール)っていう記号を使ってるから紛らわしいけど、@<code>{=}はあくまで代入を表す演算子に過ぎなくて、数学で言うところの『等しい』という意味ではないことがポイントだね。Pythonにおける代入は、あくまで@<b>{オブジェクトを参照するための名前をつけているだけ}。@<code>{y = x}は『@<code>{y}に@<code>{x}を代入する』と読んだりもするけど、その実態は『@<code>{x}(が指しているオブジェクト)に@<code>{y}という名前をつけている』、ということなんだ@<fn>{x_assign_y}。その結果として、@<code>{x}と@<code>{y}が指している値がどっちも@<code>{1}になっている、というわけだね」

//footnote[x_assign_y][方向が逆なのが大変ややこしいですね。イメージとしては@<code>{y} ← @<code>{x}で問題ないですが、実際の動きとしては@<code>{y} → @<code>{x}のほうが近いです。]

颯「うーん、まだよくわかんないかも……」

泉「図を見てもらったほうがわかりやすいかな。@<code>{x = 1}と@<code>{y = x}という操作を続けて行ったとき、『@<code>{1}を@<code>{x}でも@<code>{y}でも参照できる』、すなわち@<b>{1つのオブジェクトに2つの名前がついている}ような状態になっているんだ。要するに、オブジェクトはコピーされていない、っていうことだね」
...
りあむ「あのー、お話しているところ大変恐縮なんだけど、ぼくの画面だと@<code>{x}と@<code>{y}のidが@<code>{10750888}になっちゃってる。颯ちゃんのは@<code>{10750856}だったよね。これ、なんで違うの？」

泉「いい質問だね。@<code>{id}で表示されている値の正体は@<kw>{アドレス, address}@<fn>{address}なんだ。コンピュータには主記憶装置(メモリ)というデータを一時的に保存しておく領域があるんだけど、そのメモリ上の位置を示す値がアドレスなんだ」

//footnote[address][@<kw>{番地}ともいいます。こちらのほうがしっくりくるかもしれませんね。]

りあむ「メモリって、スマホを買うときに『メモリ: 4GB』なんて書かれてるあれのことか！主記憶装置っていうと途端にいかつく聞こえちゃうな……」

颯「アドレスっていう言葉の意味はなんとなくわかったんだけど、@<code>{id}が人によって違うのとどう関係あるの？」

泉「オブジェクトを作成したとき、その値はメモリ上の空きがある場所に書き込まれるんだけど、空いてる場所がいつも同じとは限らないんだ。だから、人によって@<code>{id}が違うのはもちろん、同じ環境であっても実行したタイミングによって変わることがあるんだ。Colabのセッションが一度切れた後、もう一度@<list>{check_id_x_and_y}を実行するとおそらく結果は変わるから、セッションが切れた後に試してみてね」
