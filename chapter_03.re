...
りあむ「もちろん答えは5だよね。……なんか、5じゃなくて5.0って出ちゃってるけど(@<list>{10_div_2})。まあ、どっちも一緒っしょ！」
...
泉「そう。10/3は3.333……とどこまでも続く数で、このように同じ数が何度も繰り返される小数を特に循環小数と呼ぶんだ。Colabで実行すると何が表示されたかな？」
...
泉「大丈夫。ここからでも入れる保険はあるよ。まず一つ考えられる手は、Colabの90分ルールを逆手に取って、ランタイムの切断を待つこと。とはいえ、流石に90分待つのは大変だから、ここではもう一つの方法、@<kw>{del文}について解説するね。といっても使い方はシンプルで、@<code>{del}に続いてスペース、そしてその後ろに削除したい変数名を書くだけ@<fn>{actual_del}。こうすることで、変数に紐づけられた値は解除されるんだ(@<list>{del_statement})@<fn>{del_rarely}」
...
//footnote[del_rarely][ちなみに、@<code>{del}文を実際のプログラミングで使う機会はほぼありません。]
...
泉「あとは画面上部の『ランタイム』から『セッションを再起動する』をクリックする方法もあるけど、どちらで対応するかはお好みで。ただ、セッションを再起動すると他の変数も全部消えちゃうからそこは注意だね」
