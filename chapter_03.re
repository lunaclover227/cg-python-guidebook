...
泉「PEP 8で示しているのは、@<ttb>{プロジェクト内で}一貫性を保つことが重要ということ。つまり、@<code>{1 + 1}と書いたすぐ下の行で@<code>{2+3}みたいな書き方をするな、ってことだね。『PEP 8にしたがったせいで逆に読みにくくなった』みたいな正当な理由があれば無視してよいとPEP 8は言ってるんだ。現状PEP 8が有名になりすぎてみんながしたがっているから私も合わせるけど、2人は書き方が一貫していれば好きなように書いて大丈夫だよ」
...
泉「お次は引き算。……と言ってもさっきとやることは変わらない。今度はこのコードを入力してみよう(@<list>{5_sub_2})。マイナスの記号『@<code>{-}』は、半角の状態でShiftキーを@<ttb>{押さずに}『ほ』って書かれてあるキーを押すと入力できるよ」
...
泉「うん。今はまだお試しだから、使わないなと思ったら消しても大丈夫。もちろん新しいセルに入力してもいいし、さっきの@<code>{1 + 1}の続きに書いてもいいよ。ただし、@<chap>{chapter_02で説明したように、@<code>{print}を使わずに複数行コードを書いた場合、最後の1つしか出力されないからそこは注意してね。@<code>{1 + 1}の前に@<code>{5 - 2}って書くと、@<code>{1 + 1}の答えが出ちゃうよ」
