...
//image[10_div_0][@<code>{10 / 0}の実行結果][scale=0.8]{
//}
...
//listnum[composed_add_and_mul_01][2 + 5 * 3を計算するコード@<fn>{composed_pep_8}]{
2 + 5 * 3
//}
...
泉「いいところに気がついたね。もちろんそのルールもあって、@<code>{(2 + 5) * 3}とすれば@<code>{2 + 5}が優先されて答えは@<code>{21}になるよ(@<list>{composed_add_and_mul_02})」

//listnum[composed_add_and_mul_02][(2 + 5) * 3を計算するコード]{
(2 + 5) * 3
//}

泉「数学では一般的に括弧を重ね書きするとき、@<code>{{\}}や@<code>{[]}を用いることがあるけど、Pythonでは括弧が何重になっても@<code>{()}を用いるから、そこだけは注意だね(@<list>{multiple_parentheses})。まあ、身も蓋もない話をするなら二重以上の括弧はわかりづらいから極力避けたほうが無難だけど……」
...
泉「前の節で、2人にはいくつか計算をしてもらったね。ただ、これまでは表示された計算結果を眺めていただけで、これだと電卓を叩いているのと一緒だったよね。今度は、一度計算した結果を繰り返し利用する方法を学んでみよう。どうすればいいと思う？」
...
//image[reference_sample_1][@<code>{a}に代入された値が表示された]{
//}
...
//image[reference_sample_2][@<code>{b}に代入された値が表示された]{
//}
