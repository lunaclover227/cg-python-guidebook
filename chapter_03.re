...
泉「お次は引き算。……と言ってもさっきとやることは変わらない。今度はこのコードを入力してみよう(@<list>{5_sub_2})。マイナスの記号『@<code>{-}』は、Shiftキーを@<b>{押さずに}『ほ』って書かれてあるキーを押すと入力できるよ」
...
泉「正解！では、このコード(@<list>{check_id_x_and_y})をセルにまとめて入力して、実行してみよう(@<img>{variable_02})」
