...
本書では、@<ruby>{Google Colaboratory, グーグルコラボラトリー}、通称@<ruby>{Colab, コラボ}を用いて学習を進めていきます。Colabはインターネットの環境とGoogleアカウントさえあれば利用可能なサービスで、煩わしい準備なしにプログラミングを始めることができます。Colabでは、セルと呼ばれる単位でコード(プログラムとして書かれた文字のこと)を書くことができ、入力したコードを実行する→結果を確かめる、というサイクルを簡単に回すことができます。もし間違えても、その場で修正して何度でも実行し直せたり、セルというごく小さな単位でコードを書くので、ミスに気付きやすく間違えた箇所を探しやすいといったメリットもあります。
 ...
 * 本書では英語の多くに振り仮名(ルビ)を振っておりますが、読みはあくまで一例となります。
