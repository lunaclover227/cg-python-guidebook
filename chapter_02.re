...
泉「複数行に渡ってコードが書けるのもセルがあるからこそだね。対話モードの基本はあくまで@<b>{一つ問いかけると一つ実行してくれる}という流れで、『コードセルに書かれたコードのうち、一番最後の行の値だけが出力される』っていう仕様も、標準の対話モードを踏襲しているから、ということなんだ@<fn>{last_row_print}」
