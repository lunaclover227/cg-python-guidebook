...
泉「これには@<kw>{文字コード}が深く関わっているんだ。文字コードはコンピュータ上で文字を表現するための規則のことで、すべての文字には文字コードにしたがった固有の識別番号が割り当てられているんだ。例えば『あ』という字は、Unicode(ユニコード)と呼ばれる文字コードでは0x3042という番号が@<fn>{unicode}、Shift-JIS(シフトジス)と呼ばれる文字コードでは0x8240という番号が割り当てられているの@<fn>{mojibake}」
...
泉「その通り！YEN SIGN問題は、もともとASCII(アスキー)という文字コードで『@<code>{\\}』に割り当てられていた0x5Cという番号を、Shift-JISで『@<code>{¥}』に割り当てたことから始まった問題なんだ。円記号は日本でこそ使われるけど、欧米では使われない文字でしょ？Shift-JISは日本語のための文字コードとして制定されたもので、このとき既にアメリカで制定されていたASCIIの『@<code>{\\}』を『@<code>{¥}』に置き換えちゃったんだ」
