...
泉「それにはいくつか理由があるんだけど、特に大きな理由を2つ説明するね@<fn>{only_python}」

=== Pythonが初学者向けである理由1: コードのシンプルさ

泉「まずは何と言ってもPythonではコードをシンプルかつ簡単に書けること。例えば以下にC言語@<fn>{c_lang}、Java@<fn>{java}という言語とPythonの3言語でそれぞれ"Hello, world!"という文字を出力するコードを示してみたんだけど、比べてみてどう？」
...
泉「りあむさんは鋭いね。1つ目の例は後で紹介する有名なプログラムだけど、2つ目の例についてはわざとPythonだと極端に短く書ける例@<fn>{list_comprehensions}を挙げているから、シンプルなのは当たり前。ただ、それでもPythonでは全体的にコードを簡潔に書ける傾向にあるのは確かで、文法も簡単だから私は初学者向けかなって思う」
...
//footnote[some_indentation][その中にもいくつか派閥(流派)があり、根深い論争を巻き起こすことがしばしばあります。このことから、決まった書き方しかできないPythonは楽という考え方もあります。]
...
りあむ「いや、流石にこんな適当に書く人はいないと思うけど……。まあ、慣れないぼくたちにとっては、決められた書き方に添えてなかったらエラーって言ってくれたほうがいいのかも」

=== Pythonが初学者向けでない理由

泉「さて、ここまでPythonが初学者向けである理由をいくつか見てきたけど、もちろんそうでない点もあるから、それも併せて紹介するね。これもいくつかあるんだけど、やっぱり一番はコードがシンプルである、ということかな」

りあむ「あれ？さっきそれ利点として聞かなかったっけ？気のせい？」

泉「コードがシンプルなのは、初学者にとって簡単である一方、十分な知識を得られないままなんとなくでコードが動いてしまうという欠点も抱えているんだ。例えば、@<list>{c_sum}や@<list>{java_sum}には@<code>{int}っていう単語が書かれてるでしょ？あれは『対象となるデータが整数ですよ』、ということを示してるんだけど、Pythonではそのことを書かないルールなんだ。Pythonには他にもこういうのがたくさんあって、その結果コードが全体を通して短く書けるということにつながっているの」

颯「これも書かなくていいならそれに越したことはないと思うんだけど、多分違うんだよね。インデントもそうだったし」

泉「もちろん意図があってそのような仕様になっているわけで、利点もたくさんあるんだけど、初めてプログラミングに触れる人が知らないまま先に進んじゃう可能性があるのがあんまりよくないかなって。さっきの整数の例とかもそうだけど、Pythonに整数の概念がないわけではないでしょ？」

りあむ「まあ……それは確かに」

泉「遅かれ早かれ、プログラミングの学習を進めていく上でそういった知識も必要になってくるから、早い段階で知っておいてもいいんじゃないかなって。だいぶ賛否両論ありそうだけどね」

泉「一つ覚えていてほしいのは、どの言語にもいいところとそうでないところがあるということ。ある言語では簡単にできる処理が他の言語ではすごく難しいっていう状況は往々にあるし、簡単にできたかと思えば処理にやたら時間がかかったりすることだってある。この言語さえマスターすれば十分、この言語さえあれば他に何もいらない、なんてことはもちろんない。今回は学びやすさの観点からPythonを選んだけど、他にも世の中にはたくさんの素敵な言語があるから、興味を持ったらぜひ学習してほしいかなって思う」
...
泉「次に、画面の左上にある『+ コード』と書かれている部分をクリックするか、マウスカーソルを画面真ん中に持っていったときに出てくる『+ コード』をクリックしてみて。どっちをクリックしても四角い入力画面が出てくると思うんだけど、これを@<kw>{セル}というんだ。ここに今からコードを書いていくよ」
...
泉「その通り！詳しくは後々順を追って説明するから、まずはセルの中にキーボードで入力してみよう。できたかな？」
...
泉「Shiftキーを押しながら2を押してみて@<fn>{double_quote}。この記号は@<kw>{ダブルクォーテーション}(または@<kw>{ダブルクォート})っていって、英語圏では台詞などを表す際に用いられるよ@<fn>{quotation_mark}」
...
泉「りあむさんも入力できた？入力が終わったら、セルの左側にある▶ボタンを押すか、Ctrlキーを押しながらEnterキーを押してみよう。どうなった？」

//image[colab_06][正しく@<code>{Hello, world!}と表示された例]{
//}
...
泉「うーん、今度はちゃんと打ててるように見えたんだけど……あっ、これもしかして『@<code>{'}』(@<kw>{シングルクォーテーション}(または@<kw>{シングルクォート}))を2個並べた？全角でも半角でも、Shiftキーと2を同時押し、は変わらないよ。正しく入力できたら、Hello, world!って文字の色が変わるはず」

りあむ「できた！！！……できたんだけど、これ、もしかして前途多難ってやつ！？なんか思ったより難しくない！？ぴえん！」

泉「私もこれまでに何度かプログラミングを教えたことがあるんだけど、文字を打つのって意外と難しいんだよね……というわけで、次のレッスンは『キーボードの使い方』！一見遠回りかもだけど、正しく文字を入力できないとプログラミングは始まらない。この機会に、まとめて勉強しちゃおう！」
