...
泉「その通り！詳しくは後々順を追って説明するから、まずはセルの中にキーボードで入力してみよう(@<img>{colab_05})。カンマ『@<code>{,}』の後にスペースを入れるかどうかはお好みで。できたかな？」
...
泉「あ、りあむさん。ダブルクォーテーションが全角文字になっちゃってる。多くのプログラミング言語では、原則全角文字は使わないんだ。後で例外も出てくるけど、基本的には半角モードに切り替えて入力するようにしてね(@<img>{change_half_width_mode})。これでどうかな？」
