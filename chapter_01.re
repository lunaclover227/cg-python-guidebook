...
泉「次に、画面の左上にある『+ コード』と書かれている部分をクリックしてみて。そうすると『コーディングを開始するか、AIで生成します。』と書かれた四角い入力画面が出てくると思うんだけど、これを@<kw>{セル}というんだ。特に、『+ コード』をクリックして挿入するのがコードを書くための@<kw>{コードセル}、対して『+ テキスト』をクリックして挿入するのが単なる文字を書くための@<kw>{テキストセル}。今回はコードセルにコードを書いていくよ」
...
颯「あれ？泉ちゃん、『@<code>{()}』や『@<code>{"}』ってどうやって打つの？」

泉「キーボードの左下のほうにある、Shiftと書かれたキー(Shiftキー)を探してみて。『@<code>{()}』はこのShiftキーを押しながら、キーボードの上のほうにある『ゆ』や『よ』と書かれたキーを押すと入力できるよ@<fn>{jis_keyboard}@<fn>{shift_key}。同様に、『@<code>{"}』はShiftキーを押しながら『ふ』と書かれたキーを押すと入力できるね。『@<code>{"}』はあまり馴染みがないかもしれないけど、@<kw>{ダブルクォーテーション}(または@<kw>{ダブルクォート})っていって、今後頻繁に出てくることになるから覚えておいてね」
...
泉「りあむさんも入力できた？入力が終わったら、セルの左側にある▶ボタンを押すか、キーボードの左下にある@<ruby>{Ctrl, コントロール}と書かれたキー(Ctrlキー)を押しながらEnterキーを押してみよう。どうなった？」
