...
泉「プログラムはコンピュータへの命令や処理を順序立てて書いたものだよ。もちろんルールにしたがって正確に書く必要はあるけど、誤解を恐れず言うならただの文字(テキスト)だね。例えばこんなものも、今回学ぶPythonで動作する立派なプログラムだよ(@<list>{1_add_1})」
...
泉「プログラミングと聞くとなんだか難しいイメージを持たれがちだけど、プログラムはパソコンでメモ帳を開いて、ささっと作ることもできるんだよ。こう聞くと少しハードルも下がったかな？ちなみに、プログラムとして書かれた文字のことを@<kw>{ソースコード, source code}、あるいは単に@<kw>{コード}というから、それも覚えておいてね。コードを書くことを@<kw>{コーディング}と言ったりもするよ」
...
泉「さて、次はプログラムの種類について見ていこう。一言でプログラムと言っても、日本語や英語みたいに様々な種類のもの(言語)が存在するんだ。これらを総称して@<kw>{プログラミング言語}というよ」

りあむ「今回勉強するのはPythonっていう言語なんだっけ？どんな言語なの？」

泉「@<kw>{Python}は1991年に開発された言語で、2025年2月現在、世界で最も利用されている言語の一つだよ(@<img>{python_org})。数値計算や画像処理、今流行りの人工知能(AI)を用いたプログラミングのほか、私たちに身近なものだとInstagram@<bib>{instagram}やSpotify@<bib>{spotify}などの開発にも利用されてたりするんだ」
...
泉「まずは何と言ってもPythonではコードをシンプルかつ簡単に書けること。例えば以下にC言語@<fn>{c_lang}、Java(ジャバ)@<fn>{java}という2つの言語とPythonでそれぞれ"Hello, world!"という文字を出力するコードを示してみたんだけど、比べてみてどう？(@<list>{c_hello}, @<list>{java_hello}, @<list>{python_hello})」
...
泉「もう一つ、同じ整数を2回かけた数@<fn>{square_number}を1から9までまとめて1つのデータに入れるコードも比べてみよう@<fn>{data_for_array}(@<list>{c_sum}, @<list>{java_sum}, @<list>{python_sum})。もちろん意味は理解できなくて大丈夫だから、今は文字の多さだけで比較してみて」
...
//footnote[tiobe_index][オランダのTIOBE Software社が毎月発表している、プログラミング言語の人気度を示す指標であるTIOBE Indexでは、それぞれPythonが1位、Javaが3位、C言語が4位に位置づけています(2025年2月現在)@<bib>{tiobe_index_source}。なお、人気度の計算には主にGoogleなどの検索エンジンにおける検索結果が用いられています。]
...
泉「続いて2つ目の理由は、@<kw>{インデント}(または@<kw>{字下げ})が意味を持つこと。どういうものかは@<img>{indentation}を見てもらったほうが早いと思う」
...
//footnote[off_side_rule][プログラミング言語が持つこのような規則を@<kw>{オフサイドルール}といいます。]
...
泉「気付いた？実はPythonに限らず、どの言語でもコードをより読みやすくするためにインデントはごく当たり前に利用されているんだ。そして、そのルールはだいたい決まってて、『ここで字下げするのが普通』っていう暗黙の了解みたいなものがあるの@<fn>{some_indentation}。ただ、インデントが自由な言語ではそこから外れても問題ないから、結果的に慣れてない人が書くとコードがガタガタになっちゃうこともある(@<list>{c_sum_lack_coherence})。一方、Pythonではそもそも正確に字下げしないとエラーになっちゃうから、慣れるのにもってこいってことだね」
...
泉「ごめんごめん、順を追って説明するね。まず、@<sec>{what_is_programming}節でプログラムの正体はただの文字だっていう話をしたと思うんだけど、それらをいきなりPythonのコードとして実行できるわけじゃなく、別途実行するための環境(開発を行う作業場)を準備しなくちゃならないんだ。大前提としてPythonそのものを用意するのはもちろん、コードを書くためのテキストエディタ@<fn>{editor}やIDE@<fn>{IDE}といったツールを揃えたり、場合によっては仮想環境@<fn>{virtual_environment}と呼ばれる環境を別途用意したりと、準備しなくちゃならないことはたくさんあるの。こういった準備のことを@<kw>{環境構築}といって、私たちがプログラミングを始める上で最初にして最大の難関って言う人もいるくらいだね」
...
泉「そこで登場するのが@<kw>{Jupyter Notebook}@<fn>{jupyter_ruby}というアプリケーション(@<img>{jupyter_notebook})。これはMicrosoft EdgeやGoogle ChromeのようなWebブラウザで扱うことができて、直感的にプログラミングを行うことができるんだ@<fn>{jupyter_support_lang}」
...
泉「じゃあ、さっそくColabにアクセスしてみよう！まずはGoogle等の検索エンジンで google colab と検索してみよう。『Colaboratoryへようこそ』っていうページが出てきたかな？@<fn>{colab_url}(@<img>{colab_01})」
...
泉「うん。そうすると、『Colabへようこそ』と書かれた画面に遷移するんだけど(@<img>{colab_02})、これはColabのチュートリアルだね@<fn>{google_login}。そこに一通り目を通してもいいんだけど、初めてだと何が書いてあるかわからないと思うから、ここでは先に進めるね。次に、左上の『ファイル』から、『ドライブの新しいノートブック』をクリックしてもらえるかな？(@<img>{colab_03})Colabで実際に編集するファイルのことを@<kw>{ノートブック}というんだけど、今からそれを作っていくよ」

//footnote[google_login][もしログイン画面が出てきた場合、お持ちのGoogleアカウントでログインしてください。ログイン方法や、アカウントの作成方法は本書では取り扱いません。]
...
泉「いま、画面の上のほうに『コーディングを開始するか、AIで生成します。』と書かれた四角い入力画面が出ているかな？もし出てなかったら、画面の左上にある『+ コード』と書かれている部分をクリックしてみて(@<img>{colab_04})。こうして挿入した領域を@<kw>{セル}というんだ。特に、『+ コード』をクリックして挿入するのがコードを書くための@<kw>{コードセル}、対して『+ テキスト』をクリックして挿入するのが単なる文字を書くための@<kw>{テキストセル}。今回はコードセルにコードを書いていくよ」

//image[colab_04][コードセルを挿入する]{
//}

泉「2人に最初に入力してもらうコードはこちら！(@<list>{python_first_hello})」
...
//footnote[shift_key][Shiftキーと@<ttb>{同時押しでない}ことに注意してください。先にShiftキーだけを押した状態で、続けて「ゆ」や「よ」を押す必要があります。]
...
泉「りあむさんも入力できた？入力が終わったら、セルの左側にある▶ボタンを押すか、キーボードの左下にあるCtrl(コントロール)と書かれたキー(Ctrlキー)を押しながらEnterキーを押してみよう@<fn>{colab_execution}。どうなった？」
...
颯「出た！Hello, world!って表示されてる！(@<img>{colab_06})」

泉「おめでとう、それが記念すべき颯ちゃんの最初のプログラムだね。ようこそ、プログラミングの世界へ！」

りあむ「まって。なんかぼくのは全然違う感じなんだけど(@<img>{colab_07})。これ絶対あってないよね？ね？？？」
...
りあむ「が～～～！！！またなんか出てきた！(@<img>{colab_08})今度はなんなんだよう！」
